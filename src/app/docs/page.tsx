...
console.log(data);`
  ...
  },
  {
    ...
    endpoints: [
      ...
      { method: "GET", path: "/api/info/provinsi", title: "Provinsi", desc: "Daftar 38 provinsi Indonesia", auth: true, curlExample: `curl "${BASE}/api/info/provinsi?apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/info/provinsi?apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data.provinsi);` },
      { method: "GET", path: "/api/info/kota", title: "Kota/Kabupaten", desc: "Daftar kota/kabupaten per provinsi", auth: true, params: "id (wajib)", curlExample: `curl "${BASE}/api/info/kota?id=31&apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/info/kota?id=31&apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data.kota_kabupaten);` },
      { method: "GET", path: "/api/info/alquran", title: "Al-Quran", desc: "Data 114 surat Al-Quran lengkap", auth: true, params: "nomor (opsional, 1-114)", curlExample: `curl "${BASE}/api/info/alquran?nomor=1&apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/info/alquran?nomor=1&apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data); // Surat Al-Fatihah` },
      { method: "GET", path: "/api/info/wilayah", title: "Wilayah Indonesia", desc: "Data wilayah lengkap: provinsi, kota/kabupaten, kecamatan, desa + GeoJSON + adjacent", auth: true, params: "type (wajib: provinces/regencies/districts/villages), id, sub, geojson, adjacent, level", curlExample: `curl "${BASE}/api/info/wilayah?type=provinces&apikey=MVAL-XXX"\ncurl "${BASE}/api/info/wilayah?type=provinces&id=11&sub=regencies&apikey=MVAL-XXX"\ncurl "${BASE}/api/info/wilayah?type=regencies&id=1101&sub=districts&apikey=MVAL-XXX"\ncurl "${BASE}/api/info/wilayah?type=districts&id=110101&sub=villages&apikey=MVAL-XXX"\ncurl "${BASE}/api/info/wilayah?type=provinces&id=11&geojson=true&apikey=MVAL-XXX"\ncurl "${BASE}/api/info/wilayah?type=regencies&id=1101&adjacent=true&level=regencies&apikey=MVAL-XXX"`, jsExample: `// Daftar semua provinsi\nconst res = await fetch("${BASE}/api/info/wilayah?type=provinces&apikey=MVAL-XXX");\nconst { data } = await res.json();\nconsole.log(data.data); // Array provinsi\n\n// Kota/kabupaten di provinsi 11 (Aceh)\nconst res2 = await fetch("${BASE}/api/info/wilayah?type=provinces&id=11&sub=regencies&apikey=MVAL-XXX");\nconst { data: kota } = await res2.json();\nconsole.log(kota.data);\n\n// GeoJSON provinsi\nconst res3 = await fetch("${BASE}/api/info/wilayah?type=provinces&id=11&geojson=true&apikey=MVAL-XXX");\nconst { data: geo } = await res3.json();\nconsole.log(geo.data); // GeoJSON FeatureCollection` },
      { method: "GET", path: "/api/info/symbols", title: "Simbol Provinsi", desc: "Lambang/simbol provinsi Indonesia", auth: true, params: "id (opsional)", curlExample: `curl "${BASE}/api/info/symbols?id=31&apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/info/symbols?id=31&apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data); // Simbol DKI Jakarta` },
      { method: "GET", path: "/api/info/postal", title: "Kode Pos", desc: "Cari data kode pos Indonesia via Nusantara API", auth: true, params: "code (wajib)", curlExample: `curl "${BASE}/api/info/postal?code=40123&apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/info/postal?code=40123&apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data); // Data kode pos` },
    ],
  },
  {
    title: "Tools",
    ...
    endpoints: [
      ...
      { method: "GET", path: "/api/tools/tracking", title: "Tracking Paket", desc: "Lacak paket ekspedisi (JNE, J&T, SiCepat, POS, TIKI, dll)", auth: true, params: "tracking (wajib), courier (opsional: jne/jnt/sicepat/pos/tiki/anteraja/lion/wahana/ninja)", curlExample: `curl "${BASE}/api/tools/tracking?tracking=JNE123456789&courier=jne&apikey=MVAL-XXX"`, jsExample: `const res = await fetch("${BASE}/api/tools/tracking?tracking=JNE123456789&courier=jne&apikey=MVAL-XXX");\nconst data = await res.json();\nconsole.log(data.data); // Status tracking` },
      { method: "POST", path: "/api/tools/nik", title: "Parse NIK", desc: "Parse NIK KTP Indonesia - dapatkan info provinsi, kota, kabupaten, kecamatan, tanggal lahir, dll", auth: true, body: `{ "nik": "1101010101010001", "reference_year": 2026, "century_override": 2000 }`, curlExample: `curl -X POST "${BASE}/api/tools/nik?apikey=MVAL-XXX" \\\n  -H "Content-Type: application/json" \\\n  -d '{"nik":"1101010101010001","reference_year":2026}'`, jsExample: `const res = await fetch("${BASE}/api/tools/nik?apikey=MVAL-XXX", {\n  method: "POST",\n  headers: { "Content-Type": "application/json" },\n  body: JSON.stringify({\n    nik: "1101010101010001",\n    reference_year: 2026\n  })\n});\nconst data = await res.json();\nconsole.log(data.data); // Info NIK lengkap` },
      { method: "GET", path: "/api/tools/myinstants", title: "MyInstants", desc: "Sound effects & meme sounds dari MyInstants", auth: true, params: "action (wajib: trending/search/detail/recent/best/uploaded/favorites), q, id, username", curlExample: `# Trending\n"${BASE}/api/tools/myinstants?action=trending&q=id&apikey=MVAL-XXX"\n\n# Search\n"${BASE}/api/tools/myinstants?action=search&q=laugh&apikey=MVAL-XXX"\n\n# Detail\n"${BASE}/api/tools/myinstants?action=detail&id=akh-26815&apikey=MVAL-XXX"\n\n# Recent\n"${BASE}/api/tools/myinstants?action=recent&apikey=MVAL-XXX"\n\n# Best\n"${BASE}/api/tools/myinstants?action=best&q=id&apikey=MVAL-XXX"\n\n# Uploaded by user\n"${BASE}/api/tools/myinstants?action=uploaded&username=hellmouz&apikey=MVAL-XXX"\n\n# Favorites\n"${BASE}/api/tools/myinstants?action=favorites&username=hellmouz&apikey=MVAL-XXX"`, jsExample: `// Trending sounds\nconst res = await fetch("${BASE}/api/tools/myinstants?action=trending&q=id&apikey=MVAL-XXX");\nconst { data } = await res.json();\nconsole.log(data.data); // Array trending sounds\n\n// Search\nconst res2 = await fetch("${BASE}/api/tools/myinstants?action=search&q=laugh&apikey=MVAL-XXX");\nconst { data: search } = await res2.json();\nconsole.log(search.data);` },
    ],
  },
  {
    title: "AM Verif",
