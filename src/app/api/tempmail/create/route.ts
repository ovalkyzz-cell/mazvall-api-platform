...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!auth) {
      return NextResponse.json({ success: false, error: 'API key tidak valid atau tidak aktif', endpoint: '/tempmail/create' }, { status: 401 });
    }
    ...
    await logApiUsage(auth.keyId, auth.user.userId, '/tempmail/create', 'GET', 200, req.headers.get('x-forwarded-for') || undefined);
    ...
    return NextResponse.json({ success: false, error: error?.message || 'Gagal generate email', endpoint: '/tempmail/create' }, { status: 500 });
  }
}
